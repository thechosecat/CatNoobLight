...
public class Cat implements Listener {
    ...
    public void fox(BlockPlaceEvent event) {
        Player player = event.getPlayer();
        if (event.getBlock().getType() != Material.LIGHT || player.getGameMode() != GameMode.SURVIVAL) {
            return;
        }
        ...
        dog.AddLoc(event.getBlock().getLocation(), player.getUniqueId());
    }
    ...
    public void dog(PlayerInteractEvent event) {
        ...
        if (block == null || block.getType() != Material.LIGHT || player.getGameMode() != GameMode.SURVIVAL) {
            return;
        }
    ...
    public void wolf(BlockBreakEvent event) {
        ...
        if (block.getType() != Material.LIGHT || event.isCancelled()) {
            return;
        }
